AFRAME.registerComponent("outline-2", {
  multiple: true,
...
//     this.quadLeft.geometry.attributes.uv.array.set([
//       0, 1, 0.5, 1, 0, 0, 0.5, 0,
//     ]);
...
//     this.quadRight.geometry.attributes.uv.array.set([
//       0.5, 1, 1, 1, 0.5, 0, 1, 0,
//     ]);
...
//   parseToken:
//     /([#a-z0-9\-\_]+)\.{0,1}([#a-z0-9\-\_]*)\s*\({0,1}\s*([\$a-z0-9\-\_\.\s]*)\){0,1}([\!\?]{0,1})/i,
...
//             fragment: prop
//               ? el[prop] instanceof Document
//                 ? el[prop].body.textContent
//                 : el[prop]
//               : el.textContent,
//             depth: el.dataset.depth !== undefined,
//             diffuse: el.dataset.diffuse !== undefined,
//             includes: el.dataset.includes
//               ? el.dataset.includes.trim().split(" ")
//               : null,
//             defaults: el.dataset.defaults
//               ? el.dataset.defaults.trim().split(" ")
//               : null,
//           };
//         } else {
...
//       }
//       if (obj.fragment) {
//         chunks.push(obj.fragment.replace(/\$/g, prefix));
//       }
//       if (obj.uniforms) {
...
//             prefix,
//             "main(",
//             swapMain ? "origColor, color" : "color, origColor",
//             ", vUv, depth);",
//           ].join("")
//         );
//       }
...
//         pickup();
//         passes.push({
//           pass: makepass(obj.material, false, obj.vr),
//           behavior: obj,
//         });
//       } else {
//         temp.push(k);
...
//     if (
//       resize &&
//       (this.needsResize || behavior.needsResize) &&
//       behavior.setSize
//     ) {
...
//       arr.push(p);
//     });
//     this.sceneEl.renderTarget =
//       arr.length && this.sceneEl.isPlaying ? rt : null;
//     this._passes = arr;
//   },
//   tock: function () {
//     var scene = this.sceneEl,
...
//       if (pass.behavior && pass.behavior.resize)
//         self.isActive(pass.behavior, true);
//       pass.pass.render(
//         renderer,
//         i < self._passes.length - 1 ? rts[(i + 1) & 1] : null,
//         r
//       );
//     });
...
//     LightState.tileData.value = [
//       width,
//       height,
//       0.5 / Math.ceil(width / 32),
//       0.5 / Math.ceil(height / 32),
//     ];
//     LightState.tileTexture.value = new THREE.DataTexture(
//       new Uint8Array(LightState.cols * LightState.rows * 4),
//       LightState.cols,
//       LightState.rows
//     );
//   },
...
//     if (camera instanceof THREE.ArrayCamera) {
//       passes = [
//         [0.5, 0, camera.cameras[0]],
//         [0.5, 0.5, camera.cameras[1]],
//       ];
//     } else {
//       passes = [1.0, 0, camera];
//     }
...
//         if (bs[1] < 0 || bs[0] > pw || bs[3] < 0 || bs[2] > LightState.height)
//           return;
//         if (bs[0] < 0) bs[0] = 0;
//         if (bs[1] > pw) bs[1] = pw;
//         if (bs[2] < 0) bs[2] = 0;
...
//           for (
//             var j = Math.floor((bs[0] + pm) / 32);
//             j <= Math.ceil((bs[1] + pm) / 32);
//             j++
//           ) {
...
//       var minX = w,
//         maxX = 0,
//         minY = LightState.height,
//         maxY = 0,
//         hw = w / 2,
//         hh = LightState.height / 2;
//       for (var i = 0; i < 8; i++) {
//         v.copy(pos);
//         v.x += i & 1 ? r : -r;
//         v.y += i & 2 ? r : -r;
